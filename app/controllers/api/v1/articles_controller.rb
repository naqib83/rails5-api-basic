  ...
  module V1
    class ArticlesController < ApplicationController
      ...
      def destroy
        article = Article.find(params[:id])
        article.destroy
        render json: {status: 'DELETED',message: 'Article deleted',data: article},
                status: :ok
      end
      ...
      def update
        ...
        if article.update_attributes(article_params)
          ...
          ender json: {status: 'ERROR',message: 'Article not updated',data: article.errors},
                  status: :unprocessble_entity
        end
      end
